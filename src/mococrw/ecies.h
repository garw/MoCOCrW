 ...
 *
 * Elliptic curve integrated encryption scheme provides you the functionality to encrypt a plaintext with an one time
 ...
 * 5. calculate the MAC over the ciphertext (with the optional salt)
 *
 * Send P_eph, ciphertext and MAC to the recipient
 *
 ...
 *    ECIESEncryptionCtx)
 * 5. Set the ephemeral key (setEphemeralKey())
 * 6. Decrypt the ciphertext using update()
 * 7. Set the MAC (setMac())
 * 8. Invoke finish for verifying the MAC and for getting the plaintext
 */
...
public:
     ...
     *
     * Default: The default MAC-Cipher is HMAC-SHA512, thus the key size is 512/8 = 64
     ...
     *
     * Default: The default symmetric cipher is AES-CBC PKCS7, thus the key length is 256/8 = 32
     * @param keySize The key size in bytes
     * @return builder instance
     */
    ...
    \endcode
     * @param func A std::function which returns an object implementing the SymmetricCipherI interface
     * @return builder instance
     */
