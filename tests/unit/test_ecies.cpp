...
auto randomKey = AsymmetricKeypair(ECCSpec().generate().internal());
...
public:
    void SetUp() override
    {
        ...
        auto cipherDecFunc = [zeroIV](const std::vector<uint8_t> &key) -> std::unique_ptr<AESCipher> {
            return AESCipherBuilder(mode, keySize, key).setIV(zeroIV).setPadding(padding).buildDecryptor();
        };

        auto cipherEncFunc = [zeroIV](const std::vector<uint8_t> &key) -> std::unique_ptr<AESCipher> {
...
protected:
    AsymmetricKeypair secp384Key = AsymmetricKeypair::generate(ECCSpec(openssl::ellipticCurveNid::SECP_384r1));
...
TEST_F(ECIESTests, testWithoutSalt)
{
    ...
    auto decCtx = decBuilder.buildDecryptionCtx(secp384Key);
    decCtx->setEphemeralKey(ephKey);
    ...
    std::vector<testData> data {
        {
            ...
            "-----END EC PRIVATE KEY-----\n",
            "0437D97E1299CF430163A11FC87B7B8FB647BD98EE8B5BAE31C3BB1EE897A77BAFA20F94F6CE0B91748996359000B"
            "47E56F2587825E4F01CEFF9BBDDE8ED17DDEE8BD46BD409DFF7FF48E9F7A869FE3F9D8174F788CD21C2D8EAE5BF25"
            "6884AA89",
            "90C4509BC7C9FFE404F6B063E2781655323BF402645C241A5F57E556AB397E6C4EFED563A77A66998A4645C7A24B82"
            "34",
            "46805E4419BEB16F7EC8D234699A1E266398819B46AD095AF7A49CC683E7561ABFB24E3DF5798B3E1451110A8F663F15"
...
-----END EC PRIVATE KEY-----
            ...
            "04EF3F43C81D752D0DA9EB550357F77864C987E0D83D834175FDD6AB6B0189D0D9D2DCB914328FE1A961F758FBEE92"
            "530EF4E2FCE19D422A85D98B61D34C87A122554F515B4587AB15F76CBC27080F6C2FEE444D6955C53D39193C23CBAA"
            "72F15B",
            "260F659ECB79C9623DEEE0A492E8A1292521C52DE98085D7BBBEBD3E73240E388AFC89F61E58B83CE60D50C9"
            "7E17B2B5",
            "DADB28BE9D7F78A02BA777E76312284B7D92D9CE9AC97967581463F76674C6837C96D74DA5472817B1740278D91164"
...
-----END EC PRIVATE KEY-----
            ...
            "0400FD13363AF3768E2AC8324FBDF23E65FA316966A67B797F6E42C3AF8DA2AFC721B809A2A91A7CD996256EDAC52A"
            "2257FAEBF13A5C09111CE8EC09C35E724EFACEA818884B962F01F4538CE31B340B3D65F75ED85BF0F8B5AF07A717F4"
            "7EC9C6",
            "552786BE64E07D0D1ABAF640A969087C15C902F5D23F173F9626C4D1B31B20C4999B8F75910228EE496E4388"
            "20B09BB0",
            "16FD040C6D66CC560B1DFE4A19997CDA850A9F0C42BAA3BED04FD00EB0932DAAA01D5F36EC91258561E8AE"
...
-----END EC PRIVATE KEY-----
            ...
            "04B00689869B4418C18B5EC8699A33984ED10CA7BBACA037A462715E841846338B4EAFF6EB93CDC9875EB1D6642E5B"
            "EB25FE994117E583DC2512C70CB83FD2B9A278FD8BA8503D27A2A96A2B17E93FE6FCA3592AD74178377E609F215428"
            "D18384",
            "4D108CEC5BF3E9FE32F5A7F98F49D0C176655DD92CE6A27197F8FA884F6A6F703ECAE253916291E01082540A"
            "CF22449E",
            "0A452DBDDD91624E4552B7DA40049251BAD5FA48C1D6D6B4F4499A918B7CFAC391A51006AA862560BBD2CC"
...
-----END EC PRIVATE KEY-----
            ...
            "0478E3A07F7111C8341A9BBC7FB979F9B1CB27CB0B20E5A0B587C9F43CE1464F53C17461E82F129684D8E6B7B06D74"
            "AD4AA590985174917E063CEE975E85DA0100626F701EC0278437466B28578852B0EB1684606BEFFF22B9FF6D591C44"
            "53111A",
            "1184FC9E16AF814F87DB3CA26F84B408DD876471215F59543D811E6C3E51BA9B0CA446440BBA8A935E99B029"
            "5708F699",
            "D29CAB796BC5900C4570B8697F41FED528BA84A339080192E31344FB6F2FF61AF05770BEBA1EA0A0C7A119"
...
void testECIES(struct testData data)
{
    ...
    auto decCtx = decBuilder.buildDecryptionCtx(privKey);
    ...
    AsymmetricPublicKey ephKey = AsymmetricPublicKey::fromECPoint(eccSpec, utility::fromHex(data.ephemeralKey));

    /* Set the ephemeral key -> calculate the shared secret */
    decCtx->setEphemeralKey(ephKey);
...
INSTANTIATE_TEST_CASE_P(ecies,
                        ECIESTest,
                        testing::ValuesIn(prepareTestDataForECIESTests()));

TEST_P(ECIESTest, testWithTestVectors)
{
    testECIES(GetParam());
}
...
TEST(ECIESBuilderTest, checkForAllNecessaryDefaultValues)
{
    ...
    ECIESCtxBuilder decBuilder;
    decBuilder.buildDecryptionCtx(randomKey);
}

TEST(ECIESBuilderTest, incompleteBuilder1)
{
    ECIESCtxBuilder decBuilder;
    std::function<std::unique_ptr<MessageAuthenticationCode>(const std::vector<uint8_t>&)> macFunc =
            [] (const std::vector<uint8_t> &key) { (void) key; return nullptr; };
    decBuilder.setMacFactoryFunction(macFunc);
    EXPECT_THROW(decBuilder.buildDecryptionCtx(randomKey), MoCOCrWException);
}
...
TEST(ECIESBuilderTest, exceptionThrowTest)
{
    ...
    ECIESCtxBuilder decBuilder;
    decBuilder.setMacKeySize(1);
    decBuilder.setMacFactoryFunction([](const std::vector<uint8_t> &key) -> std::unique_ptr<MessageAuthenticationCode> {
